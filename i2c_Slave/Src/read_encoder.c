...
void XA_config(void){
	...
	GPIO_Init(GPIO_Port_Encoder, &GPIO_InitStruct);
...
void vars_initialize(void) {
	xa = ((GPIO_Port_Encoder->IDR & GPIO_Pin_A) != (uint32_t)Bit_RESET);
	xb = ((GPIO_Port_Encoder->IDR & GPIO_Pin_B) != (uint32_t)Bit_RESET);
	stt_prev = (xa != xb) + 2 * xb;
}
